...
const LoginPage = ({ setSession }) => {
    ...
    const handleAuth = async (e) => {
        ...
        if (isSignUp && !authResponse.data.session) {
            ...
            // Si c'est une connexion réussie, l'écouteur onAuthStateChange va s'occuper du reste.
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className={`${CardStyle}`}>
                    <p className="text-sm font-medium text-gray-600">Prochaine Maintenance Planifiée</p>
                    <p className="text-3xl font-extrabold text-gray-800 mt-1">Non défini</p>
                    <p className="text-xs text-gray-500 mt-2">Ajouter un log de maintenance (dans Comptabilité).</p>
                </div>
            </div>

            <h3 className="text-2xl font-semibold text-gray-800 pt-4">Planning des Réservations</h3>
            {/* Ici pourrait être affiché un calendrier global ou une liste des 5 prochaines réservations */}
            <div className={`${CardStyle}`}>
                {reservations.length === 0 ? (
                    <p className="text-gray-500">Aucune réservation à venir. Soyez le premier !</p>
                ) : (
                    <ul className="divide-y divide-gray-100">
                        {reservations.sort((a, b) => new Date(a.date) - new Date(b.date)).slice(0, 5).map(res => (
                            <li key={res.id} className="py-3 flex justify-between items-center">
                                <div>
                                    <p className="font-semibold">{res.title}</p>
                                    <p className="text-sm text-gray-500">{res.pilot_name} &bull; {res.date}</p>
                                </div>
...
// ... (Autres vues comme ReserveView, LogFlightView, CostsView, AdminView avec les mêmes structures de formulaire et de logique)
...
const App = () => {
    ...
    const userProfile = pilots.find(p => p.id === userId);
    const isAdmin = userProfile?.is_admin || false;

    // --- LOGIQUE D'AUTHENTIFICATION SUPABASE ---
    useEffect(() => {
        if (!supabase) {
            setAuthLoading(false); // Mode Démo, pas d'auth Supabase nécessaire
            return;
        }
    ...
    useEffect(() => {
        if (supabase || userId) { // Charge les données si connecté ou si en Mode Démo
            // Abonnements aux tables (mode démo ou Supabase)
            const unsubPilots = subscribeToTable('co_owners', setPilots);
            ...
            return () => {
                if (supabase) {
                    // Nettoyage des abonnements Supabase
                    unsubPilots();
                    unsubAircraft();
                    unsubReservations();
                    unsubFlights();
                    ...
                    // Nettoyage des écouteurs d'événements de démo
                    unsubPilots();
                    unsubAircraft();
                    unsubReservations();
                    unsubFlights();
    ...
    }, [userId]); // Dépend de l'userId pour ne charger qu'après l'authentification

    // --- RENDU BASÉ SUR L'AUTHENTIFICATION ET LE MODE DÉMO ---

    if (!supabase) {
        // RENDU MODE DÉMO SANS CONNEXION
        return (
            <div className="font-sans min-h-screen bg-gray-50 flex flex-col">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
                    <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-xl mb-6 shadow-md">
                        <p className="font-bold">Mode Démo Local (Non Persistant Globalement)</p>
                        <p className="text-sm">Vos données sont enregistrées uniquement sur votre navigateur. Connectez-vous à Supabase pour partager les données.</p>
                    </div>
                    {/* Les vues sont rendues directement en mode démo pour tester la persistance locale */}
                    {view === 'dashboard' && <DashboardView user={userProfile} isAdmin={true} aircraft={aircraft} reservations={reservations} flights={flights} expenses={expenses} pilots={pilots} />}
                    {/* ... autres vues ... */}
                </div>
            </div>
        );
    ...
    if (authLoading) {
        // Écran de chargement de l'authentification Supabase
...
// ... (Les autres composants comme ReserveView, LogFlightView, CostsView, AdminView sont omis ici pour la concision, mais doivent être inclus dans le fichier réel)
